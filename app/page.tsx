    ...
    <>
      ...
      <main className="dark:bg-linear-to-bl  from-[#000000] to-[#474747] from w-full h-auto min-h-full flex flex-col items-center justify-center">
        <div className="w-60 flex justify-between mb-3">
          <button onClick={() => setMode(true)} className={`cursor-pointer text-white hover:bg-[#88D752] rounded-[3px] w-[35%]  ${Mode == true ? 'CircleLR' : ''}`}><span className="shadow-text">LOGIN</span></button>
          <button onClick={() => setMode(false)} className={`cursor-pointer text-white hover:bg-[#88D752] rounded-[3px] w-[35%] ${Mode == true ? '' : 'CircleLR'}`}><span className="shadow-text">CADASTRO</span></button>
        </div>
        <div className={`shadow-Page border-black border-[1px] bg-[#FFFFFF] text-black rounded-[8px] w-60 h-96 ${Mode == true ? 'max-h-64 scalaReverse' : 'scala'}`}>
          {Mode == true ? (
            <form className="flex flex-col w-full h-full mt-4 ml-5" action="">
              ...
              <button className="bg-[#88D752] text-white w-26 rounded-[3px] mx-13 mt-2 cursor-pointer hover:bg-[#5f963b]">LOGAR</button>
            </form>
          ) : (
            <form className="flex flex-col w-full h-full mt-4 ml-5" action="">
              <label className="block mb-2">Usuario</label>
              ...
              <button onClick={(e) => { e.preventDefault();/*Evita recarregamento da página*/ RegisterUser(User, Userconf, Senha, Senhaconf) }} className="bg-[#88D752] w-26 rounded-[3px] text-white mx-13 mt-2 cursor-pointer hover:bg-[#5f963b]">CADASTRAR</button>
            </form>
          )}
        </div>
